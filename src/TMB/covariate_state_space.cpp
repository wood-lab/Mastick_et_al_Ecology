...
Type objective_function<Type>::operator() ()
{
  ...
  DATA_VECTOR(xtobs);
  DATA_IVECTOR(xyearindex);
  DATA_SCALAR(nx);
  DATA_SCALAR(priorlogsigmaobs_mu);
  DATA_SCALAR(priorlogsigmaobs_sigma);
  ...
  PARAMETER_VECTOR(vxt); // vector of the vxt's
  PARAMETER(logsigma_vxt);
  PARAMETER(logsigma_xobs);
  PARAMETER(xo);
  PARAMETER(thetax);
  
  Type jnll = 0.0;
  jnll -= dnorm(logsigma_xobs, priorlogsigmaobs_mu,priorlogsigmaobs_sigma, true);
  ...
  Type sigma_vxt = exp(logsigma_vxt);
  Type sigma_xobs = exp(logsigma_xobs);
  vector<Type> xt(nyears);
  ...
  for (int i = 0; i < nx; i++) {
    jnll -= dnorm(xtobs(i), xt(xyearindex(i)), sigma_xobs,true);
  }
  ADREPORT(xt);
  
  rt(0) = vt(0) + thetax * xt(0);
  lognt(0) = logno;
  nt(0) = exp(lognt(0));
  
  for ( int i  = 1; i < nyears_minus_1; i++){ 
    rt(i) = rt(i - 1) * rho + pow(1 - pow(rho, 2), -2) * vt(i) + thetax * xt(i);
  }
  ...
  jnll -= sum(dnorm(vxt, 0, sigma_vxt, true)); 
